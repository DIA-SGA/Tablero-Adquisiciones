...
tablafinalEP<-tablafinalEP %>% 
  ...
  mutate(reparticion.actual.agrup = case_when(
    `repartición actual`=="DGPFE" ~ "DGPFE",
    `repartición actual`=="DAFYP" ~ "DGPFE",
    ...
    `repartición actual`=="DSB" ~ "Programas",
    `repartición actual`=="DSD" ~ "Programas",
    `repartición actual`=="DSFYTT" ~ "Programas",
    `repartición actual`=="DSO" ~ "RRHH",
    `repartición actual`=="DGO" ~ "DGA",
    `repartición actual`=="DGPYCP" ~ "Presupuesto",
    TRUE                      ~ "Programas")) 

unique(tablafinalEP$reparticion.actual.agrup)
...
tablafinalEP <- tablafinalEP %>% 
  ...
  filter(NumExpediente!="EX-2021-54031992-APN-DCYC#MS") %>%
  filter(NumExpediente!="EX-2021-55628332-APN-DCYC#MS") %>%
  filter(NumExpediente!="EX-2021-76260838-APN-DCYC#MS") %>%
